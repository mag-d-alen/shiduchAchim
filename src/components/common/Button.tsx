...
type ButtonProps = {
  text: string;
  color: 'primary' | 'secondary' |'destroy' ;
  size?: 'sm' | 'md' | 'lg';
  onClick?: () => void;
};

export const Button: React.FC<ButtonProps> = ({ text, color = 'primary', size = 'md', onClick }) => {
  const sizeClasses = {
  ...
  const colorClasses = {
    ...
    'destroy': 'color6 text-white'
  };


  const getButtonClasses = () => `${colorClasses[color]} ${sizeClasses[size]} items-center justify-center font-medium border box-border flex gap-x-2 outline-none transition-colors disabled:cursor-not-allowed cursor-pointer round-button`
